...
#testList = (["100K"])#,"10K","100K"])
#testTime = ([150*n*nPi,160*n*nPi,200*n*nPi,500*n*nPi])
#testTime = ([150*n*nPi,160*n*nPi,200*n*nPi,500*n*nPi])
#testTime = ([1300+500*(n)*(nPi)])
...
toFileList = ["server pi-1  192.168.1.36:80\n", 
              "server pi-2  192.168.1.74:80\n", 
              "server pi-3  192.168.1.94:80\n",
              "server pi-4  192.168.1.210:80\n",
              "server pi-5  192.168.1.81:80\n",
              "server pi-6  192.168.1.161:80\n",
              "server pi-7  192.168.1.175:80\n",
              "server pi-8  192.168.1.135:80\n",
              "server pi-9  192.168.1.237:80\n",
              "server pi-10 192.168.1.98:80\n"]
def power_on_pi(p):
...
def update(u2):
    ...
    time.sleep(260)
    print("waiting for it tshould have finished, e.g. -fin1-.")
def nginx(u):
...
def get_data(pi,test):
    for jk in range(pi):
        ...
        time.sleep(2)    
        p = Popen(["scp -i ssh-key-for-fruit -r root@"+ipList[jk]+":/root/ /home/lunet/eljb13/Dropbox/rpi_testbed/"+testList[test]+"/pi"+str(jk+1)], stdin=PIPE, shell=True)
        print "getting files"
        time.sleep(100)
...
def run_ab():
    ...
    for ab in range(len(testList)):
        ...
        time.sleep(20)
        os.system("sudo python3 power_off3B.py")
        #time.sleep(70)
