function filedata = getOdroidDataTime(x,y,test,type,pi)
    ...
    for j = 1:x
        for i = 1:y
            for k = 1:(j)
                for l = 1:pidLength
                    count=count+1;
                    filename = sprintf("/home/lunet/eljb13/Dropbox/rpi_testbed/"+pi+"/"+type+"/"+test+"/pi"+num2str(k)+"/data/" + num2str(nginxPids(k,l)) +"time%dPI%d.txt",(j-1),(i-1));
                    if isfile(filename)                                   
                        ...
                        filedata{count} = {'16:39:09.986';'16:39:11.131'};
                    end                    
