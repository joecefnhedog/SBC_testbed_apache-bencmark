...
def run_command(i,j):
    ...
    for p in range(len(processes)):
        process = (subprocess.Popen('(top -b -n 1 | grep '+str(processes[p])+' | head -1 | awk \'{print $1 " " $9 " " $10 " " $11}\') >> data/'+str(processes[p])+'htop-i'+str(i) +'PI'+str(j) +'.txt',shell=True))
        process = (subprocess.Popen('date +\"%T.%3N\" >> '+'data/' +str(processes[p])+'time'+str(i) +'PI'+str(j)+'.txt',shell=True))
    #time.sleep(sleepT)
    return process
def get_data():
    global currentData
...
if not os.path.exists(folder):
    os.mkdir(folder)
    os.system("sudo chmod 777 "+folder+"/")
