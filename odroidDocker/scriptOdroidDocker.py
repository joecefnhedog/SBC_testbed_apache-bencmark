...
#testTime = ([750+110*(n)*(nPi)])
testTime = ([750+120*(n)*(nPi),900+140*(n)*(nPi),1000+170*(n)*(nPi),1500+230*(n)*(nPi)])
def send_script(i):
...
#need to fix the process id's part, we are pulling two id's from the nginx process. 
...
def get_data(pi,test):
    for jk in range(pi):
        ...
        time.sleep(2)    
        p = Popen(["sudo sshpass -p odroid scp -o StrictHostKeyChecking=no -r odroid@"+ipList[jk]+":/home/odroid/data /home/lunet/eljb13/Dropbox/rpi_testbed/odroidDocker/"+testList[test]+"/pi"+str(jk+1)], stdin=PIPE, shell=True)
        #p = Popen(["sshpass -p odroid ssh -o StrictHostKeyChecking=no odroid@" +ipList[m]+" 'nohup python local_script.py "+ipList[m]+"' "], stdin=PIPE, shell=True)
        print "getting files"
        time.sleep(40)
